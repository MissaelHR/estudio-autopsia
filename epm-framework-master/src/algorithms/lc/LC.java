...
import PRFramework.Core.SupervisedClassifiers.EmergingPatterns.Miners.LCMiner;
...
public class LC extends LC_Wrapper
{

    public String name = "LC";
    ...
    public int[] maxVariabilityPerLevel = new int[]{5, 4, 3, 2};
    ...
    public LC (InstanceSet train, HashMap<String, String> params)
    {
        this.train = train;
        ...
        String[] levels;
        levels = params.get("maxVariabilityPerLevel").split(",");
        int[] l = new int[levels.length];

        for (int i = 0; i < levels.length; i++) {
            l[i] = Integer.parseInt(levels[i]);
        }
        maxVariabilityPerLevel = l;
    }
    ...
    public void mine ()
    {
        ...
        miner.setMaxVariabilityPerLevel(maxVariabilityPerLevel);
