...
/**
 * {@link ARFFDataConverter} creates a ({@link DataRecordSet} given a
 * {@link Reader}
 * 
 * @author atzmueller
 */
public class ARFFDataConverter {
	...
	List<Attribute> attributes = readARFFHeader(tokenizer);
	...
	// complete building String attributes:
	for (Attribute att : attributes) {
	    if (att instanceof StringAttribute) {
		((StringAttribute)att).buildValues();
	    }
	}
    ...
    private List<Attribute> readARFFHeader(StreamTokenizer tokenizer)
	    throws IOException {
	...
	if (ARFF_RELATION_START.equalsIgnoreCase(tokenizer.sval)) {
	    ...
	    ReaderUtils.throwError(tokenizer, "keyword " + ARFF_RELATION_START
		    + " expected");
	}
	...
	while (ARFF_ATTRIBUTE_START.equalsIgnoreCase(tokenizer.sval)) {
		...
		if (tokenizer.sval.equalsIgnoreCase(ARFF_ATTRIBUTE_TYPE_REAL)
			|| tokenizer.sval
				.equalsIgnoreCase(ARFF_ATTRIBUTE_TYPE_INTEGER)
			|| tokenizer.sval
				.equalsIgnoreCase(ARFF_ATTRIBUTE_TYPE_NUMERIC)) {
			...
			.equalsIgnoreCase(ARFF_ATTRIBUTE_TYPE_DATE)) {
		    String format = null;
		...
		// skip the case that no nominal values exist
		if (attributeValues.size() == 0) {
		    // ReaderUtils
		    // .throwError(tokenizer, "no nominal values found");
		    attributeValues.add(EMPTY_ATTRIBUTE_VALUE);
		    basicBuilder.buildNominalAttribute(attributeName,
			    attributeValues);
		    basicBuilder.buildEmptyNominalValues();
		} else {
		    basicBuilder.buildNominalAttribute(attributeName,
			    attributeValues);
	...
	if (!ARFF_DATA_START.equalsIgnoreCase(tokenizer.sval)) {
	    ReaderUtils.throwError(tokenizer, "keyword " + ARFF_DATA_START
		    + " expected");
	}
