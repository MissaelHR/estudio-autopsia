 ...
 * 
 */
public class StandardQF extends AbstractQFSimpleStatisticBased implements
	...
	super();
	this.a = a;
	name = "Standard quality function, a=" + a;
    }
	...
	return (Math.pow(subgroupSize, a) * (sgMean - populationMean));
    }

    @Override
    public double evaluateBin(double subgroupSize, double tp,
	    double definedPopulationCount, double populationPositives) {
	return (Math.pow(subgroupSize, a) * ((tp / subgroupSize) - (populationPositives / definedPopulationCount)));
    }
