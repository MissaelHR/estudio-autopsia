...
public class HotellingTSquareBasedQF implements IQualityFunction {
	...
	return new HotellingTSquareBasedQF(a);
    }
	...
	numericTargetAttributeValuesMeansInPopulation = new double[attributes
		.size()];
	int pos = 0;
	for (Attribute attribute : attributes) {
	    int size = 0;
	...
	for (int i = 0; i < attributes.size(); i++) {
	    meanDiff[i] = numericTargetAttributeValuesMeans[i]
		    - numericTargetAttributeValuesMeansInPopulation[i];
	}
