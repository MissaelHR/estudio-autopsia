...
public abstract class AbstractQFStatisticBased implements
	IQualityFunctionStatisticBased {
	...
	return evaluate(sg.getStatistics());
    }

    @Override
    public boolean isApplicable(ISubgroup<DataRecord> subgroup) {
	if (subgroup instanceof SG) {
	    return isApplicable((SG) subgroup);
	}
	return false;
    }

    public abstract boolean isApplicable(SG subgroup);

    @Override
    public abstract AbstractQFStatisticBased clone();
}
